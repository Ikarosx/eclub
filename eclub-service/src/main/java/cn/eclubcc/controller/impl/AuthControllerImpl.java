...
public class AuthControllerImpl implements AuthController {
  ...
  /**
   * 小程序授权之后应该携带code和加密数据请求此接口 在此接口中携带AppId、AppSecret和code访问微信服务器 微信服务器返回session_key和openId
  ...
  /**
   * 用户通过getOpenIdByCode后得到了openid 然后在前端通过getUserInfo接口拿到用户数据 再带着用户数据和签名请求此接口进行用户信息的录入
   *
